...
async def test_live_traffic_rewrite_of_an_entry_opened_on_an_apple_device_gives_it_a_fresh_identity(
    hass: HomeAssistant,
    source_calendar: FakeCalendar,
    dav_server: FakeDav,
    waze: FakeWaze,
    ...
    412 on an entry someone opened, so the entry is written under a new name and the old one deleted, as the README's
    limits say. No extra Waze call is made for it."""
    source_calendar.events = [match()]
    entry = waze_entry()
    await setup_entry(hass, entry)
    [href] = dav_server.puts()
    dav_server.open_on_device(href)
    ...
    new_href = hass_storage[STORE_KEY]["data"]["events"][event_key(match())]["href"]
    assert new_href != href
    assert dav_server.calls == [("PUT", href), ("PUT", new_href), ("DELETE", href)]
